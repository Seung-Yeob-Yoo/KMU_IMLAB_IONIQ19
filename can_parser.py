...
class Can_parser:
    ...
    def get_can_data(self):
        ...
        if vehicle == 'NE':
            can_bus = can.interface.Bus(channel = 'can0', bustype = 'socketcan',bitrate=500000, data_bitrate=2000000, fd=True, can_filters=filters) # HMC NE
        elif vehicle == 'IONIQ19':
            can_bus = can.interface.Bus(channel = 'can0', bustype = 'socketcan', can_filters=filters) # KMU
        else:
...
if __name__ == '__main__':
    ...
    Can_parser(vehicle=vehicle, can_db_path=can_db_path, can_data_name=can_msg).get_can_data()